    ...
    .then((body) => {
      ...
      parseString(body, function (err, result) {
        ...
        for (i = 0; i < nbPage; i++) {
          console.log(i)
          msg.reply.text(
            result.feed.entry[i].title[0] +
              '\nAuthor: ' +
              result.feed.entry[i].author[0].name[0] +
              '\n' +
...
/*
    1. On obtient le fichier distant
    2. On prendre la première entrée du fichier distant
    3. On regarde si elle existe dans le fichier local
    3a. Si oui, on ne fait rien du tout
    3b. Si non, on remplace le fichier local avec le fichier distant
*/
async function cronWork() {
  ...
  if (
    !(
      reference.length == tmpArray.length &&
...
reference = [
  'https://about.gitlab.com/blog/2020/09/10/cloud-native-storage-beginners/',
